/*
 * delivery.c - Fresh Picks: Post-Order Delivery Management (v5 — Direct Indexing)
 * ================================================================================
 * Refactored from v4: SLL linear scans replaced with O(1) pointer-table lookups.
 *
 * ARCHITECTURE:
 *   - main() loads ALL SLLs once, builds pointer tables, passes them down.
 *   - cmd_ functions never call load_*/free_* — they receive live table pointers.
 *   - Orders and DeliveryBoys use direct-index tables keyed by get_index_from_id().
 *   - Full-scan commands (get_active_orders, batch_promote_slot, list_*) still
 *     walk the SLL — O(1) tables only benefit point-lookup commands.
 *   - All SLL frees and table frees happen at the bottom of main(), once.
 *
 * COMMANDS (argv[1]):
 ...
 *
 * OUTPUT FORMAT: SUCCESS|... or ERROR|...
