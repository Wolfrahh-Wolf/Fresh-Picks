/*
 * users.c - Fresh Picks: User Management Logic (v5 — Direct Indexing)
 * ===========================================================================
 * Called by Flask via subprocess.run(): ./users <action> [arg1] [arg2] ...
 *
 * ARCHITECTURE: SLLs are the persistent source of truth. The pointer table
 * (UserNode**) is built once in main() for O(1) lookups by user_id.
 * All load/free calls live exclusively in main(); cmd_ functions receive
 * the pre-built table and its size. No cmd_ function touches the SLL directly.
 *
 ...
 *
 * STATUS NOTE: Derived from password field — non-empty = "Active", else "Inactive".
 *
...
void cmd_search_users(const char* query,
    ...
    for (int i = 0; i < user_table_size; i++) {
        ...
        str_to_lower(id_lower,   user_table[i]->data.user_id,   MAX_ID_LEN);
        str_to_lower(name_lower, user_table[i]->data.full_name, MAX_STR_LEN);
        if (strcmp(id_lower, query_lower) == 0 || strstr(name_lower, query_lower))
            match_count++;
    }
